...
总结上述内容，我们在图 \ref{fig:single-server} 中展示了我们的单服务器可验证PIR方案，其中验证过程以蓝色标记突出显示。
...
\begin{figure*}
    ...
    \textbf{单服务器协议}
        ...
        \begin{itemize}
            ...
            \item \textbf{Reconstruct:} $\client$ 重构出记录 $\db_\dbidx \coloneqq  \sumhint_\hintidx-(\sumanswer-\crumbvalue_{\blockidx, \crumbidx})$。 \bluetext{$\client$ 验证是否有 $\randomhint_q-(\randomanswer-r\cdot \crumbvalue_{\blockidx, \crumbidx}) = Eval(\randomset_\hintidx, \blockidx)\cdot \db_\dbidx$。如果验证失败，$\client$ 输出 $\bot$。否则， } $\client$ 输出 $\db_\dbidx$。
            \item \textbf{Refresh:}
                  \begin{itemize}
