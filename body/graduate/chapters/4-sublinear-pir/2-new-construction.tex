...
Crumb不会影响客户端计算答案。每个Crumb仅包含一条已知的记录，因此可以从答案中去除Crumb的值。假设$\answer$是来自服务器的答案，$\crumbvalue$是所用Crumb的值，答案可以由客户端计算为$\answer \coloneqq \answer - \crumbvalue$。最后，客户端可以按照原本协议中的方式利用$\answer$重构记录$\db_\dbidx$。
